...
export default function BeepHero() {
  ...
  // Handle keyboard toggle: open once then close on second tap
  ...
  // AI Orb: only opens when orb button is clicked
        ...
        if (typeof window !== 'undefined' && window.speechSynthesis) {
          ...
          utter.onend = () => {
            setTimeout(() => {
              ...
              aiUtter.rate = 0.9;
              aiUtter.pitch = 0.85;
              const aiVoice = voices.find(v => v.name.toLowerCase().includes('google') && v.lang === 'en-US') ||
  ...
  return (
    <section id="hero" className="min-h-screen pt-24 pb-16 px-6 md:px-12 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #f5f4fa 0%, #edeaf7 50%, #f0eef8 100%)' }}>
        ...
        <div className="flex-shrink-0 flex items-center justify-center float-element" style={{ '--dur': '6s', '--delay': '0.5s' } as React.CSSProperties}>
          <div className="relative" style={{ width: 300, height: 620 }}>
            ...
            <div
              ...
              style={{ background: '#201e38', border: '2px solid rgba(255,255,255,0.12)', boxShadow: '0 32px 80px rgba(124,95,240,0.3), 0 0 0 1px rgba(124,95,240,0.15)' }}
            >
              {/* Notch */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-24 h-6 bg-[#201e38] rounded-b-2xl z-20" />
              ...
              {/* Chat area */}
              <div ref={chatRef} className="overflow-y-auto px-3 py-3 space-y-2" style={{ height: keyboardOpen ? 240 : 420, scrollbarWidth: 'none', transition: 'height 0.3s ease' }}>
                  ...
                  if (msg.type === 'user') {
                    return (
                      <div key={msg.id} className="flex justify-end chat-bubble-enter">
                        <div className="bg-[#7c5ff0] text-white text-[12px] rounded-2xl rounded-br-sm px-3 py-2 max-w-[200px] leading-relaxed" style={{ fontFamily: 'Geist, sans-serif' }}>
                          {msg.text}
                        </div>
                      </div>
                    );
                    ...
                    return (
                      <div key={msg.id} className="chat-bubble-enter">
                        <div className="bg-white/8 border border-white/10 rounded-2xl p-3 max-w-[230px]">
                          ...
                          <div className="mt-2 text-[#9b7ff8] text-[15px] font-bold">₹5,100</div>
              ...
              {/* Voice overlay */}
              {showVoice && (
                <div className="absolute inset-0 bg-[#16142a] flex flex-col items-center justify-center z-30">
                  ...
                  <p className="text-[#9b7ff8] text-[14px] font-semibold" style={{ fontFamily: 'Geist, sans-serif' }}>{LANGUAGES[currentLang]}</p>
                </div>
              )}

              {/* Bottom input bar + optional keyboard */}
              <div className="absolute bottom-0 left-0 right-0 z-10">
                {/* Input bar */}
                <div className="px-3 pb-2 pt-2 bg-gradient-to-t from-[#16142a] via-[#16142a] to-transparent">
                  <div className="flex items-center gap-2">
                    {/* Clicking input area toggles keyboard */}
                    <button
                    ...
                    {/* Static AI Orb chip — clicking opens full-screen AI orb */}
                    <button
                      onClick={handleOrbChipClick}
                      suppressHydrationWarning
                      className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 cursor-none"
                      style={{ background: 'radial-gradient(circle at 35% 35%, #9b7ff8, #7c5ff0, #2a1a8e)', boxShadow: '0 0 10px rgba(124,95,240,0.5)' }}
                      title="Click to open AI Orb"
                    >
                      <div className="flex gap-0.5 items-center">
                        {[0,1,2].map(i => (
                          <div key={i} className="w-0.5 h-2 bg-white/80 rounded-full" />
                        ))}
                      </div>
                    </button>
                  </div>
                ...
                {/* Keyboard — only visible when keyboardOpen */}
                {keyboardOpen && (
                  <div className="bg-[#1e1b35] px-1.5 pb-2 pt-1.5" style={{ animation: 'chatBubbleIn 0.25s ease forwards' }}>
                    {/* Row 1 */}
                    <div className="flex gap-1 mb-1 justify-center">
                      {['q','w','e','r','t','y','u','i','o','p'].map((k) => (
                        <div key={k} className="flex-1 bg-[#2d2b45] rounded text-white/60 text-[9px] text-center py-1.5 font-medium" style={{ fontFamily: 'Geist, sans-serif' }}>{k}</div>
                      ))}
                    </div>
                    {/* Row 2 */}
                    ...
                    {/* Row 3 */}
                    <div className="flex gap-1 mb-1 justify-center">
                      <div className="bg-[#3d3b58] rounded text-white/40 text-[8px] text-center py-1.5 px-2">⇧</div>
                      {['z','x','c','v','b','n','m'].map((k) => (
                        <div key={k} className="flex-1 bg-[#2d2b45] rounded text-white/60 text-[9px] text-center py-1.5 font-medium" style={{ fontFamily: 'Geist, sans-serif' }}>{k}</div>
                      ))}
                      <div className="bg-[#3d3b58] rounded text-white/40 text-[8px] text-center py-1.5 px-2">⌫</div>
                    ...
                    <div className="flex gap-1 justify-center">
                      <div className="bg-[#3d3b58] rounded text-white/40 text-[8px] text-center py-1.5 px-2">123</div>
                      <div className="flex-1 bg-[#2d2b45] rounded text-white/30 text-[9px] text-center py-1.5">space</div>
                      <div className="bg-[#7c5ff0] rounded text-white text-[8px] text-center py-1.5 px-2">return</div>
                    </div>
                  </div>
                )}
              </div>
      ...
      {/* Full-screen AI Orb overlay — only opens when orb button is clicked */}
      {showAIOrb && (
        <div
          className="fixed inset-0 z-[99990] flex flex-col items-center justify-center"
          style={{ background: 'rgba(10,8,22,0.97)', backdropFilter: 'blur(20px)' }}
        >
          {/* Close button */}
          <button
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {Array.from({ length: 20 }).map((_, i) => (
              <div
                key={i}
                className="absolute rounded-full bg-[#7c5ff0] particle-rise"
                style={{
                  left: `${(i * 17 + 5) % 95}%`,
                  bottom: `${(i * 13 + 5) % 80}%`,
                  width: 2 + (i % 3),
          ...
          <div className="relative mb-10">
            ...
            {[1, 2, 3, 4].map(i => (
              <div
                ...
                style={{
                  inset: `-${i * 20}px`,
                  border: `1px solid rgba(124,95,240,${0.3 - i * 0.06})`,
            ...
            <div
              ...
              style={{
                background: orbPhase === 'human' ?'radial-gradient(circle at 35% 35%, #e8a87c, #d4845a, #8b4513)' :'radial-gradient(circle at 35% 35%, #9b7ff8, #7c5ff0, #2a1a8e)',
                boxShadow: orbPhase === 'human' ?'0 0 60px rgba(212,132,90,0.5), 0 0 120px rgba(212,132,90,0.2)' :'0 0 60px rgba(124,95,240,0.6), 0 0 120px rgba(124,95,240,0.25)',
          ...
          <div className="mb-4">
            {orbPhase === 'human' ? (
              ...
              <span className="text-[12px] tracking-[0.2em] uppercase text-[#9b7ff8] font-medium" style={{ fontFamily: 'Geist, sans-serif' }}>
                beep AI · en-US
              </span>
            )}
          </div>
          ...
          <div className="max-w-[480px] px-8 text-center min-h-[80px]">
            ...
            {orbPhase === 'ai' && aiText && (
              <p className="text-[#b09af8] text-[16px] leading-relaxed" style={{ fontFamily: 'Instrument Serif', fontStyle: 'italic' }}>
